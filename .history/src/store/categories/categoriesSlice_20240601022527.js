...
export const createCategory = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    console.log(args);
    try {
      const { data } = await axios.post("http://localhost:8000/api/v1/category", args);
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
    }
...
export const updateCategory = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    try {
      const { data } = await axios.put(`http://localhost:8000/api/v1/category/${args._id}`, {
        title:args.title
      });
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
...
export const deleteCategory = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    try {
      const { data } = await axios.delete(`http://localhost:8000/api/v1/categories/${args}`);
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
    }
