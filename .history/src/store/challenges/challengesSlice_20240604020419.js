  ...
  async (args, thunkApi) => {
    try {
      ...
      const { data } = await axios.get(
        `https://bookyin.onrender.com/api/v1/header`
...
export const createChallenge = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    ...
    try {
      const { data } = await axios.post("https://bookyin.onrender.com/api/v1/header", {
        title:args.title,
  ...
  async (args, thunkApi) => {
    try {
      const { data } = await axios.put(
        `https://bookyin.onrender.com/api/v1/challenges/${args.id}`,
        args.values
      );
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
  ...
  async (args, thunkApi) => {
    ...
    try {
      const { data } = await axios.delete(
        `https://bookyin.onrender.com/api/v1/header/${args._id}`
      );
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
