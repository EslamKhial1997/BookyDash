...
export const getChallenges = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:8000/v1/categorieschallenges?page=${page}&size=${size}&search=${search}${filter}`);
...
export const updateChallenge = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    try {
      const { data } = await axios.put(`/challenges/${args.id}`, args.values);
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
    }
