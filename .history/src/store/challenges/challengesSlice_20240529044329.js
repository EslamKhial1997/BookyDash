...
export const getChallenges = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:8000/v1/challenges?page=${page}&size=${size}&search=${search}${filter}`);
...
export const createChallenge = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    console.log(args);
    try {
      const { data } = await axios.post("/challenges", args);
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
    }
...
export const updateChallenge = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    try {
      const { data } = await axios.put(`http://localhost:8000/v1/challenges/${args.id}`, args.values);
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
    }
...
export const deleteChallenge = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    try {
      const { data } = await axios.delete(`http://localhost:8000/v1/challenges/${args.id}`);
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
    }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateChallenge.fulfilled, (state, { payload }) => {
        state.isLoading = false;
        const indexAt = state.data.findIndex(el => el._id === payload.challenge._id);
      ...
      .addCase(deleteChallenge.fulfilled, (state, { payload }) => {
        state.isLoading = false;
        state.data = state.data.filter(el => el._id !== payload.challenge._id);
        state.itemsCount--;
        state.error = null;
      })
