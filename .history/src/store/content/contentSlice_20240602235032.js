...
export const getconten = createAsyncThunk(
  "conten/getconten",
  async (args, thunkApi) => {
    try {
...
export const updateChallenge = createAsyncThunk(
  "conten/updateChallenge",
  async (args, thunkApi) => {
    try {
      const { data } = await axios.put(
        `http://localhost:8000/v1/conten/${args.id}`,
        args.values
      );
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
...
const contenSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getconten.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(getconten.fulfilled, (state, { payload }) => {
        console.log(payload);
      ...
      .addCase(getconten.rejected, (state, { payload }) => {
        state.isLoading = false;
